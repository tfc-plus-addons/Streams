...
import com.dunk.tfc.WorldGen.SpawnerAnimalsTFC;
...
public class TFCChunkProviderGenerate extends ChunkProviderGenerate
{
	...
	public Chunk provideChunk(int chunkX, int chunkZ)
	{
		...
		if(this == ((WorldServer)worldObj).theChunkProviderServer.currentChunkProvider)
            RiverGenerator.surfaceWaterGenerator().generate(worldObj.provider, chunkX, chunkZ, idsBig, metaBig);
        else
            return new Chunk(this.worldObj, idsBig, metaBig, chunkX, chunkZ);
        //delvr end
	...
	private void replaceBlocksForBiomeHigh(int chunkX, int chunkZ, Block[] idsTop, Random rand, Block[] idsBig, byte[] metaBig)
	{
		...
		for (int xCoord = 0; xCoord < 16; ++xCoord)
		{
			for (int zCoord = 0; zCoord < 16; ++zCoord)
			{
				...
				float rain = rainfallLayer[arrayIndexDL] == null ? DataLayer.RAIN_400.floatdata1 : rainfallLayer[arrayIndexDL].floatdata1;
